...
void init_crc2(std::string fname)
{
	ifstream f(fname);

	if (f.is_open() == false)
	{
		std::cerr << "File 'hashed_names.txt' not found in executable's path.\n";
		cin.get();
		exit(0);
	}
...
std::string getexepath()
{
    ...
    std::string output = std::string(result, GetModuleFileName(NULL, result, MAX_PATH));
...
{
	init_crc2(getexepath() + "hashed_names.txt");

	if (argc < 1)
	{
		std::cerr << "Usage: aamp <filename.bin>\n";
		return 1;
	}
...
void to_xml(string fname)
{
	xmlDoc.Clear();
	file = ifstream(fname, ifstream::binary);
	...
	file.close();
	XMLError eResult = xmlDoc.SaveFile(string(string(fname) + ".xml").c_str());
}
